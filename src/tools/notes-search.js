...
function score(item, query) {
  ...
  for (const queryToken of queryTokens) {
    if (haystack.includes(queryToken)) result += 4;
    const candidates = haystack.split(/[^a-z0-9]+/).filter(Boolean);
    const best = candidates.reduce((highest, candidate) => Math.max(highest, similarity(queryToken, candidate)), 0);
...
function ranked(items, query, maxResults) {
  return items
    .map((item, index) => ({ item, index, score: query ? score(item, query) : 0 }))
    .filter(({ score }) => !query || score > 0)
    .sort((left, right) => right.score - left.score || left.index - right.index)
    .slice(0, maxResults)
    .map(({ item, score: relevance }) => ({ ...item, ...(query ? { relevance: Number(relevance.toFixed(3)) } : {}) }));
}
